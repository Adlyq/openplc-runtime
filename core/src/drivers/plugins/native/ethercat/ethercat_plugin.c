...
static void rebase_module_channel_offsets(ecat_master_instance_t *inst)
{
    ...
    if (config->slave_count != 1)
        return;
    const ecat_slave_t *slave = &config->slaves[0];
    ...
    for (int p = 0; p < slave->tx_pdo_count; p++)
    ...
    for (int k = 0; k < inst->channel_map.input_count; k++)
        inst->channel_map.inputs[k].iomap_offset += bin;
}
...
static int start_single_master(ecat_master_instance_t *inst)
{
    ...
    /* Re-base module channel offsets to the real SM layout (fixed prefix). */
    ...
    /* Senmun-style modular gateways clear their 0x8000 port configuration on
     * every mapping generation (every PRE-OP->SAFE-OP).  Now that we are
