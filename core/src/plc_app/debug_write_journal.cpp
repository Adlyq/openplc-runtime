...
bool located_journal_type(uint8_t area, uint8_t size, journal_buffer_type_t *out)
{
    ...
    {
    case SZ_BIT:
        *out = is_in ? JOURNAL_BOOL_INPUT : is_out ? JOURNAL_BOOL_OUTPUT : JOURNAL_BOOL_MEMORY;
        return true;
    case SZ_BYTE:
        *out = is_in ? JOURNAL_BYTE_INPUT : JOURNAL_BYTE_OUTPUT;
        return true;
    case SZ_WORD:
        *out = is_in ? JOURNAL_INT_INPUT : is_out ? JOURNAL_INT_OUTPUT : JOURNAL_INT_MEMORY;
        return true;
    case SZ_DWORD:
        *out = is_in ? JOURNAL_DINT_INPUT : is_out ? JOURNAL_DINT_OUTPUT : JOURNAL_DINT_MEMORY;
        return true;
    case SZ_LWORD:
        *out = is_in ? JOURNAL_LINT_INPUT : is_out ? JOURNAL_LINT_OUTPUT : JOURNAL_LINT_MEMORY;
        return true;
    default:
        return false;
    }
...
void journal_write_located(journal_buffer_type_t type, uint8_t size, uint16_t idx, uint8_t bit,
                           uint64_t v)
{
    switch (size)
    {
    case SZ_BIT:
        journal_write_bool(type, idx, bit, (v & 1) != 0);
        break;
    case SZ_BYTE:
        journal_write_byte(type, idx, (uint8_t)v);
        break;
    case SZ_WORD:
        journal_write_int(type, idx, (uint16_t)v);
        break;
    case SZ_DWORD:
        journal_write_dint(type, idx, (uint32_t)v);
        break;
    case SZ_LWORD:
        journal_write_lint(type, idx, v);
        break;
    default:
        break;
    }
...
void apply_located(const DbgwEntry *e, uint8_t area, uint8_t size, uint16_t byte_index,
                   uint8_t bit_index)
{
...
extern "C" int runtime_external_write(uint8_t arr, uint16_t elem, uint8_t op, const uint8_t *bytes,
                                      uint16_t len)
{
    ...
    if (n >= DBGW_MAX_ENTRIES)
    {
        ...
        if (!g_overflow_logged)
        {
            log_warn("[debug-write] queue full (%zu) — external write dropped; "
                     "applied next cycle once it drains",
                     DBGW_MAX_ENTRIES);
...
extern "C" void debug_write_journal_drain(void)
{
    ...
    if (g_dbgw_count.load(std::memory_order_acquire) == 0)
        return;
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        const bool located =
            ...
            ext_strucpp_debug_locate(e->arr, e->elem, &area, &size, &byte_index, &bit_index) != 0;
